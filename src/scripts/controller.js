...
function handleClickOnStars() {
  $("body").on("click keypress", ".stars", function (e) {
    ...
    if (!store.items.adding) {
      ...
      let item = store.findById(cuidId);
      api
        .findAndUpdate(cuidId, { rating: store.items.currentRating.selected })
        .then(() => {
...
function generateEntryTemplate(item) {
  ...
  if (item.rating != undefined || item.editingRating) {
    ...
    for (let i = 0; i < 5; i++) {
      if (
        i <
        (item.editingRating ? store.items.currentRating.rating : item.rating)
      )
        star = "full_star.png";
      else star = "emptystar2.png";
      newEntry += `<img class ="stars" src="photos/${star}" id="n${
        i + 1
      }"}></img>`;
    }
  } else {
...
function generateExpandedView(item) {
  ...
  if (!item.editing) newEntry += `<p>${item.desc}</p>`;
    ...
    newEntry += `<textarea class="editing">${item.desc}</textarea>
...
<input type="textbox" id="inputbookmark" class="inputbm" placeholder="Place link here..." value=${
    $(".inputbm").val() || ""
  }>
<div class="entry-selected" style="border: none"> 
<label for="title">Place title here</label>
<input id="title" type="textbox" class="title" placeholder="Title of Page" value=${
    $(".title").val() || ""
  }>
<label for="rate">Rate this bookmark</label>
<span id="rate">`;
  for (let i = 0; i < 5; i++) {
    if (i < store.items.currentRating.rating)
      newCreationTemplate += `<input name="rating" type="image" class="stars" id="n${
        i + 1
      }" src="photos/full_star.png">`;
    else
      newCreationTemplate += `<input type="image" class="stars" id="n${
        i + 1
      }" src="photos/emptystar2.png">`;
  }

  newCreationTemplate += `</span>
<label for="description">Describe this bookmark</label>
<textarea id="description" class="description" placeholder="Add a description (optional)">${
    $(".description").val() || ""
  }</textarea>
...
function handleDoneEditing() {
  $("body").on("click", ".done-editing", function (e) {
    ...
    api.findAndUpdate(id, { desc: description }).then((res) => {
      item.editing = false;
...
function handleNewBookmarkSubmit() {
  $("body").on("submit", ".new-bookmark", function (e) {
    ...
    let rating;
    if (store.items.currentRating.selected > 0)
      rating = store.items.currentRating.selected;
    //figure out how to make rating optional
    let title = $(".title").val();
    store.items.error = null;
    api
      .addBookmark(newURL, description, rating, title)
      .then((data) => {
        data.expanded = false;
        data.editing = false;
  ...
  api
    ...
    .then((data) => {
      data.forEach((current) => {
        store.items.bookmarks.push(current);
      });
      render();
    })
...
function render() {
  ...
  } else $("main").html(generateNewCreationTemplate());
}
